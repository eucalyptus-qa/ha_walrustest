...
def parsein()
	...
	parts = line.split(/Bucket: (.*) Object: (.*)\n/)
    end
    return parts
end
...
begin
    bucketname = parts[1]
    objectname = parts[2]
    bucket = RightAws::S3::Bucket.create(s3, bucketname, false, 'public-read', :location => :us)
    screen_dump("Get Object: #{bucketname}/#{objectname}")
    object = RightAws::S3::Key.create(bucket, objectname)
