...
post_ops_mod_euca_conf_extra_touch($DISTRO, $VERSION, $ARCH, $SOURCE, $ROLL);
...
### handle any extra modification requested on the MEMO
sub post_ops_mod_euca_conf_extra_touch{
	...
	if( is_disable_tunneling_from_memo() == 1 ){
		my_sed( "# DISABLE_TUNNELING=\"N\"", "DISABLE_TUNNELING=\"Y\"", $euca_conf );
	};
...
sub is_disable_tunneling_from_memo{
	if( $ENV{'QA_MEMO'} =~ /DISABLE_TUNNELING=(.+)\n/ ){
		my $extra = $1;
		$extra =~ s/\r//g;
		print "FOUND in MEMO\n";
		print "DISABLE_TUNNELING=$extra\n";
		if( $extra eq "YES" ){
			$ENV{'QA_MEMO_DISABLE_TUNNELING'} = $extra;
			return 1;
		}else{
			return 0;
		};
